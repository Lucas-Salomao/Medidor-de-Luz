...
static float voltage_samples[MOVING_AVERAGE_SIZE] = {0.0f};
...
float BAT_Get_Volts(void)
{
    ...
    if (do_calibration1_chan3) {
        ...
        float current_voltage = (float)(voltage[0][0] * 3.0 / 1000.0) / Measurement_offset;

        // Update moving average
        voltage_sum -= voltage_samples[sample_index];
        voltage_samples[sample_index] = current_voltage;
        voltage_sum += current_voltage;
        sample_index++;
        if (sample_index >= MOVING_AVERAGE_SIZE) {
            sample_index = 0;
        }
        if (sample_count < MOVING_AVERAGE_SIZE) {
            sample_count++;
        }

        if (sample_count > 0) {
            BAT_analogVolts = voltage_sum / sample_count;
        } else {
            BAT_analogVolts = 0;
        }
    }
