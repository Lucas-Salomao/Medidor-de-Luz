...
{
	uint8_t buf[7] = {0};
	// Substituído ESP_ERROR_CHECK por verificação manual para evitar abort em caso de erro I2C
	esp_err_t ret = I2C_Read(PCF85063_ADDRESS, RTC_SECOND_ADDR, buf, 7);
	if (ret != ESP_OK) {
		ESP_LOGE("PCF85063", "I2C read failed: %s", esp_err_to_name(ret));
		return;  // Mantém os valores anteriores em caso de erro
	}
