...
void clock_settings_screen_load(void) {
    ...
    // Se a tela de configuração do relógio já existir, a exclui para garantir a recriação
    if (clock_config_screen != NULL) {
    ...
    // Cria a tela de configurações do relógio e a atribui à variável global
    clock_config_screen = lv_obj_create(NULL);
    lv_obj_t *scr = clock_config_screen; // Usa a variável local 'scr' para facilitar
