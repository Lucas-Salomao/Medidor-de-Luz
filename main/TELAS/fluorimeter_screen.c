...
static TaskHandle_t zero_task_handle = NULL;
...
static void update_battery_indicator(lv_timer_t *timer);
...
static esp_err_t perform_measurement_sampling(float *final_avg_lux, const char* base_message, string_id_t progress_string_id);
...
static void update_ui_for_state(screen_state_t new_state) {
    ...
    switch (current_state) {
        ...
        case STATE_MEASURE_PROMPT:
            ...
            lv_label_set_text_fmt(value_label, "%s 0.00", get_string(STRING_LUX_PREFIX));
            lv_obj_clear_flag(measure_btn, LV_OBJ_FLAG_HIDDEN);
            Play_Music(audio_dir, AUDIO_MEASURE_PROMPT);
            break;
        ...
        case STATE_SHOW_RESULT:
            ...
            Play_Music(audio_dir, AUDIO_RESULT_PROMPT);
            break;
    }
}
...
static void id_kb_confirm_event_handler(lv_event_t *e) {
    ...
    if (f == NULL) {
        ...
        char audio_dir[64];
        snprintf(audio_dir, sizeof(audio_dir), "/sdcard/%s", get_language_code());
        Play_Music(audio_dir, AUDIO_SAVE_SUCCESS);
    }
...
static void zero_sensor_task(void *pvParameters) {
    float avg_lux;
    if (perform_measurement_sampling(&avg_lux, get_string(STRING_ZEROING), STRING_ZEROING_PROGRESS) == ESP_OK) {
        lux_offset = avg_lux;
        ...
        if(err_data){
            ...
            strncpy(err_data->text, get_string(STRING_ERROR_ZEROING), sizeof(err_data->text) - 1);
            err_data->text[sizeof(err_data->text) - 1] = '\0';
            lv_async_call(async_update_label_cb, err_data);
        }
        vTaskDelay(pdMS_TO_TICKS(2000));
        lv_async_call(async_set_ui_state, (void*)STATE_ZEROING_PROMPT);
    }
    zero_task_handle = NULL;  // Libera o handle antes de deletar
    vTaskDelete(NULL);
}

static void measure_sensor_task(void *pvParameters) {
    float avg_lux;
    if (perform_measurement_sampling(&avg_lux, get_string(STRING_MEASURING), STRING_MEASURING_PROGRESS) == ESP_OK) {
        ...
        if(err_data){
            err_data->label = value_label;
            strncpy(err_data->text, get_string(STRING_READ_ERROR), sizeof(err_data->text) - 1);
            err_data->text[sizeof(err_data->text) - 1] = '\0';
            lv_async_call(async_update_label_cb, err_data);
        }
    }
    measure_task_handle = NULL;  // Libera o handle antes de deletar
    vTaskDelete(NULL);
}
...
static void zero_btn_event_handler(lv_event_t *e) {
    ...
    // Evita criar múltiplas tasks se o botão for clicado rapidamente
    if (zero_task_handle != NULL) {
        ESP_LOGW(TAG_MAIN_SCREEN, "Zero task already running, ignoring click");
        return;
    }
    xTaskCreate(zero_sensor_task, "zero_task", 4096, NULL, 5, &zero_task_handle);
}
...
static void measure_btn_event_handler(lv_event_t *e) {
    ...
    if (measure_task_handle != NULL) {
        ESP_LOGW(TAG_MAIN_SCREEN, "Measure task already running, ignoring click");
        return;
    }
    xTaskCreate(measure_sensor_task, "measure_task", 4096, NULL, 5, &measure_task_handle);
}
...
static void update_battery_indicator(lv_timer_t *timer) {
    ...
    int percentage = (int)((voltage - 3.2f) * 100.0f);
    ...
    lv_label_set_text_fmt(battery_label, "%s %d%%", battery_icon, percentage);
}
...
void create_main_screen(lv_obj_t *parent) {
    ...
    // Só cria a task se ainda não existir (evita duplicação na troca de idioma)
    if (datetime_task_handle == NULL) {
        xTaskCreate(datetime_update_task, "datetime_update_task", 4096, NULL, 5, &datetime_task_handle);
    }
    
    // Só cria o timer se ainda não existir
    if (battery_timer == NULL) {
        battery_timer = lv_timer_create(update_battery_indicator, 60000, NULL);
    }
}

// Função para limpar recursos antes de reinicializar a UI (chamada na troca de idioma)
void cleanup_fluorimeter_resources(void) {
    ESP_LOGI(TAG_MAIN_SCREEN, "Limpando recursos do fluorimeter para troca de idioma");
    
    // Deleta a task datetime se existir
    if (datetime_task_handle != NULL) {
        vTaskDelete(datetime_task_handle);
        datetime_task_handle = NULL;
        ESP_LOGI(TAG_MAIN_SCREEN, "datetime_update_task deletada");
    }
    
    // Deleta o timer de bateria se existir
    if (battery_timer != NULL) {
        lv_timer_del(battery_timer);
        battery_timer = NULL;
        ESP_LOGI(TAG_MAIN_SCREEN, "battery_timer deletado");
    }
    
    // Invalida os ponteiros UI estáticos (serão recriados com a nova tela)
    datetime_label = NULL;
    battery_label = NULL;
    ...
    // Limpa ponteiros do modal se existirem
    if (id_modal_bg != NULL) {
        // O modal será deletado junto com a tela pai
        id_modal_bg = NULL;
        id_modal_cont = NULL;
        id_input_textarea = NULL;
        keyboard = NULL;
