...
static const char *TAG_WIFI_SETTINGS = "WIFI_SETTINGS_SCREEN";
#define AUDIO_WIFI_CONNECTED    "wifi_connected.mp3"
#define AUDIO_WIFI_ERROR        "wifi_error.mp3"

// static lv_obj_t *wifi_screen = NULL; // Removido para usar a variável global wifi_config_screen
...
static TaskHandle_t scan_task_handle = NULL;  // Handle para evitar múltiplos scans simultâneos
...
void wifi_settings_screen_init(void) {
    if (wifi_config_screen == NULL) { // Alterado de wifi_screen para wifi_config_screen
        wifi_config_screen = lv_obj_create(NULL); // Alterado de wifi_screen para wifi_config_screen
        lv_obj_set_style_bg_color(wifi_config_screen, lv_color_white(), LV_PART_MAIN);

        // Adiciona evento de clique na tela para fechar o teclado
        lv_obj_add_event_cb(wifi_config_screen, screen_click_event_handler, LV_EVENT_CLICKED, NULL);
        ...
        // Back button
        lv_obj_t *home_btn = lv_btn_create(wifi_config_screen);
        ...
        // SSID Dropdown
        ssid_dropdown = lv_dropdown_create(wifi_config_screen);
        lv_obj_set_size(ssid_dropdown, 200, 40);
        lv_obj_align(ssid_dropdown, LV_ALIGN_TOP_LEFT, 20, 80);
        
        // Refresh button
        lv_obj_t *refresh_btn = lv_btn_create(wifi_config_screen);
        ...
        // Password Label
        lv_obj_t *pwd_label = lv_label_create(wifi_config_screen);
        ...
        // Password Input
        password_input = lv_textarea_create(wifi_config_screen);
...
void wifi_settings_screen_load(void) {
    // Se a tela já existir, destrua-a para forçar a recriação com o novo idioma
    if (wifi_config_screen != NULL) { 
        lv_obj_del(wifi_config_screen);
        wifi_config_screen = NULL; // Garante que a tela será recriada
        
        // Nullifica ponteiros estáticos para evitar dangling pointers
        ssid_dropdown = NULL;
        password_input = NULL;
        status_label = NULL;
        keyboard = NULL;
    ...
    lv_scr_load_anim(wifi_config_screen, LV_SCR_LOAD_ANIM_FADE_ON, 500, 0, false); 
}

static void back_btn_event_handler(lv_event_t *e) {
    ESP_LOGI(TAG_WIFI_SETTINGS, "Back button clicked");
...
static void connect_btn_event_handler(lv_event_t *e) {
    ESP_LOGI(TAG_WIFI_SETTINGS, "Connect button clicked");
    ...
    ESP_LOGI(TAG_WIFI_SETTINGS, "Connecting to SSID: %s with password: %s", selected_ssid, password);
...
static void refresh_btn_event_handler(lv_event_t *e) {
    ESP_LOGI(TAG_WIFI_SETTINGS, "Refresh button clicked");
    
    // Evita criar múltiplas tasks de scan se o botão for clicado rapidamente
    if (scan_task_handle != NULL) {
        ESP_LOGW(TAG_WIFI_SETTINGS, "Scan already in progress, ignoring click");
        return;
    }
...
static void scan_wifi_task(void *pvParameters) {
    ...
    scan_task_handle = NULL;  // Libera o handle antes de deletar
    vTaskDelete(NULL);
}
...
static void populate_wifi_dropdown(void *data) {
    ...
    if (buffer == NULL) {
        ESP_LOGE(TAG_WIFI_SETTINGS, "Failed to allocate memory for dropdown options");
    ...
    ESP_LOGI(TAG_WIFI_SETTINGS, "Populated dropdown with %d networks", WIFI_NUM);
}

static void password_input_event_handler(lv_event_t *e) {
    ESP_LOGI(TAG_WIFI_SETTINGS, "Password input clicked");
    // Mostra o teclado
    if (keyboard != NULL) {
...
static void screen_click_event_handler(lv_event_t *e) {
    ...
    if (target != password_input && keyboard != NULL && !lv_obj_has_flag(keyboard, LV_OBJ_FLAG_HIDDEN)) {
        ...
        ESP_LOGI(TAG_WIFI_SETTINGS, "Keyboard hidden due to screen click");
    }
}
...
static void keyboard_close_handler(lv_event_t *e) {
    ...
    if (keyboard != NULL) {
        lv_obj_add_flag(keyboard, LV_OBJ_FLAG_HIDDEN);
        ESP_LOGI(TAG_WIFI_SETTINGS, "Keyboard hidden due to OK button");
    }
}
