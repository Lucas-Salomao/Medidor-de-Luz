...
uint16_t WIFI_Scan(void) {
    ...
    // Substituído ESP_ERROR_CHECK por verificação manual para evitar abort em caso de erro
    esp_err_t ret = esp_wifi_scan_start(&scan_config, true);
    if (ret != ESP_OK) {
        ESP_LOGE("WIFI", "Scan start failed: %s", esp_err_to_name(ret));
        WIFI_NUM = 0;
        WiFi_Scan_Finish = 1;
        return 0;
    }
    
    ret = esp_wifi_scan_get_ap_num(&WIFI_NUM);
    if (ret != ESP_OK) {
        ESP_LOGE("WIFI", "Get AP num failed: %s", esp_err_to_name(ret));
        WIFI_NUM = 0;
        WiFi_Scan_Finish = 1;
        return 0;
    }
    ...
    ret = esp_wifi_scan_get_ap_records(&WIFI_NUM, ap_records);
    if (ret != ESP_OK) {
        ESP_LOGE("WIFI", "Get AP records failed: %s", esp_err_to_name(ret));
        WIFI_NUM = 0;
        WiFi_Scan_Finish = 1;
        return 0;
    }
